    ...
    public class RegisterModel : PageModel
    {
        ...
        public async Task OnGetAsync(string returnUrl = null)
        {
            ...
            else
            {
                ...
                {
                    RoleList = _roleManager.Roles
                        .Where(x => x.Name != StaticDetails.Role_Admin)
                        .Select(x => x.Name).Select(i => new SelectListItem { Text = i, Value = i })
                };
            }

            if (!Input.RoleList.Any())
            {
                Input.RoleList = new List<SelectListItem>
                {
                    new SelectListItem { Text = StaticDetails.Role_Vendor, Value = StaticDetails.Role_Vendor },
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    if (!await _roleManager.RoleExistsAsync(StaticDetails.Role_Admin))
                    {
                        ...
                        await _roleManager.CreateAsync(new IdentityRole(StaticDetails.Role_Vendor));
                    }

                    if (Input.Role == StaticDetails.Role_Vendor)
                    {
                        await _userManager.AddToRoleAsync(user, StaticDetails.Role_Vendor);
                    }
                    else if (Input.Role == StaticDetails.Role_Admin)
                    {
            ...
            {
                Input.RoleList = _roleManager.Roles.Where(x => x.Name != StaticDetails.Role_Admin).Select(x => x.Name).Select(i => new SelectListItem { Text = i, Value = i });
            }
